...
describe('Article retrieval', () => {
  it('should retrieve a provision by document_id and section', () => {
    const row = db.prepare(
      "SELECT content FROM legal_provisions WHERE document_id = 'ma-code-penal-cybercrime' AND section = '607-3'"
...
describe('Search', () => {
  it('should find results via FTS search', () => {
    const rows = db.prepare(
      "SELECT COUNT(*) as cnt FROM provisions_fts WHERE provisions_fts MATCH 'portant'"
    ).get() as { cnt: number };
...
describe('Negative tests', () => {
  ...
  it('should return no results for invalid section', () => {
    const row = db.prepare(
      "SELECT COUNT(*) as cnt FROM legal_provisions WHERE document_id = 'ma-code-penal-cybercrime' AND section = '999ZZZ-INVALID'"
    ).get() as { cnt: number };
    expect(row.cnt).toBe(0);
  });
});
...
describe('All 10 laws are present', () => {
  const expectedDocs = [
    'ma-code-penal-cybercrime',
    'ma-dgssi-critical-infrastructure',
    'ma-loi-05-20',
    'ma-loi-09-08',
    'ma-loi-17-97',
    'ma-loi-24-96',
