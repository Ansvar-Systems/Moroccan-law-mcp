/**
 * search_eu_implementations — Search EU directives/regulations referenced by Moroccan legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_moroccan_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as moroccan_statute_count
  ...
  if (input.has_moroccan_implementation) {
    sql += ' HAVING moroccan_statute_count > 0';
  }
