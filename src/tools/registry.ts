/**
 * Tool registry for Moroccan Law MCP Server.
...
import { getMoroccanImplementations, type GetMoroccanImplementationsInput } from './get-moroccan-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including the Bulletin Officiel du Royaume du Maroc. ' +
    ...
    'Call this FIRST when you need to understand what Moroccan legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Moroccan statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Moroccan statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Moroccan?" ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether a Moroccan statute or provision is currently in force, amended, repealed, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get international legal instruments that a Moroccan statute references or aligns with. ' +
      'Moroccan laws may reference international standards and conventions ' +
      'where applicable. ' +
      'Returns EU document identifiers, reference types, and implementation status.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Moroccan statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_moroccan_implementations',
    description:
      'Find all Moroccan statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Moroccan statutes. ' +
      'Moroccan may reference international legal instruments where relevant.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Moroccan legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_moroccan_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Moroccan legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU legal basis for a SPECIFIC provision within a Moroccan statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU alignment status for a Moroccan statute or provision. ' +
      ...
      'Moroccan is an EU Member State; this checks implementation status and cross-references.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Moroccan statute identifier.' },
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_moroccan_implementations':
          result = await getMoroccanImplementations(db, args as unknown as GetMoroccanImplementationsInput);
          break;
