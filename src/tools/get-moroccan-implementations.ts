/**
 * get_moroccan_implementations — Find Moroccan statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetMoroccanImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface MoroccanImplementationResult {
  document_id: string;
...
export async function getMoroccanImplementations(
  db: InstanceType<typeof Database>,
  input: GetMoroccanImplementationsInput,
): Promise<ToolResponse<MoroccanImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as MoroccanImplementationResult[];
