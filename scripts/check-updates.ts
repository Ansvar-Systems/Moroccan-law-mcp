#!/usr/bin/env tsx
/**
 * Moroccan Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://adfrfrfrala.justice.gov.ma';
const PORTAL_NAME = 'Adala (Moroccan Justice Portal)';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/moroccan-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Moroccan Law MCP — Data Freshness Check');
