#!/usr/bin/env tsx
/**
 * Database builder for Moroccan Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Moroccan Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Moroccan statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'MA');
    insertMeta.run('source', 'official-moroccan-government-sources');
